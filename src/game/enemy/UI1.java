 ...
 *
 */
public class UI1 implements UI {
	...
	public void update(List<Planet> planets) {
		...
		if (cta.isEmpty()) {
			...
			for(Planet planet : planets) {
				if (planet.getOwnerTeam() == player.getTeam()) {
					
					planetFrom = planet;
					for(Planet planetEnemy : planets) {
						if (planetEnemy.getOwnerTeam() != player.getTeam()) {
					...
					if (planetFrom != null && planetTo != null && planetFrom.getId() != planetTo.getId()) {
						...
						// Send standard attack
							...
							if (planetFrom.getPopulation() > planetFrom.getPopulationMax()/(Config.rs.nextInt(agression)+2)) {
